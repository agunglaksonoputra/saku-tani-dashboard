        ...
        {options.length > 0 && (
          <Popover>
            ...
            <PopoverContent className="p-1 w-full max-w-[300px]">
              {options.map((option) => (
                <button type="button" key={option.id} onClick={() => onOptionSelect(option.name)} className="w-full text-left px-2 py-1 hover:bg-muted rounded text-sm cursor-pointer">
                  {option.name}
                </button>
              ))}
            </PopoverContent>
